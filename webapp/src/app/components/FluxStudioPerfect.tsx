...
interface FluxStudioProps {
  onBackToHome: () => void;
  onNavigate: (page: string) => void;
}

export function FluxStudioPerfect({ onBackToHome, onNavigate }: FluxStudioProps) {
  // API Key State
  const [fluxApiKey, setFluxApiKey] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (!key) {
      ...
      const demoKey = 'r8_demo_xxxxxxxxxxxxxxxxxxxxxxxxxxxx';
      localStorage.setItem('flux_api_key', demoKey);
      setFluxApiKey(demoKey);
    } else {
      setFluxApiKey(key);
    }
  }, []);
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Mock API call - replace with real Flux API
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-gray-50 via-white to-orange-50/30 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 relative overflow-hidden flex">
        {/* Left: CONTROLS Panel - EXACT from Google */}
            ...
            <div className="px-6 py-6 space-y-4 max-h-[calc(100vh-180px)] overflow-y-auto">
              ...
              <div className="bg-gray-50/50 rounded-2xl p-4">
                ...
                <textarea
                  ...
                  style={{ border: 'none', boxShadow: 'none' }}
                />
                <p className="mt-2 text-xs text-orange-600 font-medium flex items-start gap-1">
                  <span>💡</span>
                  <span>Flux excels at rendering text in images! Try "sign that says..."</span>
                </p>
              </div>
              ...
              <button
                ...
                className="w-full py-4 rounded-xl bg-gradient-to-r from-orange-500 to-yellow-500 text-white font-black text-base hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {isGenerating ? (
                  <>
        ...
        {/* Right: Canvas/Gallery Area */}
        <div className="flex-1 bg-gray-50 overflow-hidden flex items-center justify-center relative">
          {generatedImages.length === 0 ? (
            // Empty State
            <div className="text-center max-w-2xl px-8">
              <div className="w-24 h-24 rounded-full bg-gradient-to-br from-orange-500 to-yellow-500 flex items-center justify-center mx-auto mb-6 shadow-xl shadow-orange-500/30">
                <Zap className="w-12 h-12 text-white" strokeWidth={2} />
              </div>
              <h2 className="text-3xl font-black bg-gradient-to-r from-orange-600 to-yellow-600 bg-clip-text text-transparent mb-3">
                Professional Image Generation
              </h2>
              <p className="text-content-faint font-medium text-lg mb-8">
                Create stunning photorealistic images with Flux Pro's state-of-the-art AI
              </p>
              <div className="grid grid-cols-3 gap-4">
                <div className="p-4 bg-white border-2 border-orange-200 rounded-2xl">
                  <p className="text-xs font-bold text-orange-600 mb-1">STEP 1</p>
                ...
                <div className="p-4 bg-white border-2 border-orange-200 rounded-2xl">
                  <p className="text-xs font-bold text-orange-600 mb-1">STEP 2</p>
                  <p className="text-sm font-black text-gray-900 mb-1">Configure</p>
                  <p className="text-xs text-content-faint">Choose settings</p>
                </div>
                <div className="p-4 bg-white border-2 border-orange-200 rounded-2xl">
                  <p className="text-xs font-bold text-orange-600 mb-1">STEP 3</p>
                  <p className="text-sm font-black text-gray-900 mb-1">Generate</p>
                  <p className="text-xs text-content-faint">Get results instantly</p>
                </div>
              </div>
            </div>
          ) : (
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {generatedImages.map((image, index) => (
                        <div
                          key={index}
                          className="group relative aspect-square rounded-xl overflow-hidden bg-gray-100 cursor-pointer hover:ring-2 hover:ring-orange-500 transition-all"
                        >
                          <img
                            src={image}
